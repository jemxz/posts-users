...
module.exports = async function getAbout(page, nameOfPage) {
  let about = [];
  ...
  try {
    console.log("StartedAboutPage");
    let options = { button: "middle" };
    await page.goto("https://m.facebook.com/" + nameOfPage + "/about");
  } catch (error) {
  ...
  try {
    ...
    for (let i = 0; i < totalNames; i++) {
      var basicInfo = await page.evaluate(
        (l, sel) => {
          let elements = Array.from(document.querySelectorAll(sel));
  ...
  try {
    await page.goto("https://m.facebook.com/" + nameOfPage);
  } catch (error) {
    logToFile(error.message, "../../../../logs/FacebookusersError.log");
    return console.log(error.message);
  }
